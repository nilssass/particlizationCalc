...
global_title = r'standard + shear contribution, $\epsilon_{fo}=0.5$ GeV/fm$^3$'
...
print(np.add(phi[:,0],0.5*dphi), '\n\n')
print(PzPhiPlot)
plt.grid()

# Save the plot data to an external file
data = np.column_stack((np.add(phi[:, 0], 0.5 * dphi), PzPhiPlot))
np.savetxt('data_pz_standard.txt', data)
...
#title=[r'$P^x$',r'$P^J_\varpi$',r'$P^z_\varpi$']
#title=[r'$P^x$',r'$P^J_\sigma$',r'$P^z_\sigma$']
title=[r'$P^x$',r'$P^J_\varpi+P^J_\sigma$',r'$P^z_\varpi+P^z_\sigma$']
