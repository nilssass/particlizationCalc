...
namespace hydro
{
  ...
  double shear_tensor(const element *surf_element, int mu, int nu)
  {
    ...
    const double shear = 0.5 * (surf_element->dmuCart[mu][nu] + surf_element->dmuCart[nu][mu] - term_3 - term_4 - term_5 - term_6 + term_7) - (1. / 3.) * (gmunu[mu][nu] - u[mu] * u[nu]) * (term_10 - term_11);
  ...
  void doCalculations(int pid)
  {
    ...
    //**************************************************************
    const std::string coefficient_filename = "params/coeffData.csv";
    const std::string output_filename = "params/interpolationTable.txt";
...
#pragma omp parallel for
    ...
    { // loop over all elements
      ...
      if (fabs(surf[iel].dbeta[0][0]) > 1000.0)
        nBadElem++;
      ...
      for (int ipt = 0; ipt < pT.size(); ipt++)
        for (int iphi = 0; iphi < phi.size(); iphi++)
        {
          ...
          const double mutot = surf[iel].mub * baryonCharge + surf[iel].muq * electricCharge + surf[iel].mus * strangeness;
            ...
            for (int nu = 0; nu < 4; nu++)
              for (int rh = 0; rh < 4; rh++)
                for (int sg = 0; sg < 4; sg++)
                {
                  ...
                  Pi_num[ipt][iphi][mu] += pds * nf * levi(mu, nu, rh, sg) * p_[sg] * surf[iel].dmuCart[nu][rh] / surf[iel].T;
                  ...
                  for (int ta = 0; ta < 4; ta++)
                  {
                    ...
                    {
                      Pi_num_navierstokes[ipt][iphi][mu] += pds * nf * ((xi_delta_coefficient * beta * beta) / z) * beta * levi(mu, nu, rh, sg) * u_[nu] * p_[rh] * gmunu[sg][alph] * shear_tensor(&surf_element, ta, alph) * p_[ta];
                    }
                  }
  ...
  void calcInvariantQuantities()
  {
    ...
    { // loop over all elements
      ...
      for (int mu = 0; mu < 4; mu++)
        for (int nu = 0; nu < 4; nu++)
        {
          symm_deriv += 0.25 * pow(hbarC * (surf[iel].dbeta[mu][nu] + surf[iel].dbeta[nu][mu]), 2) * gmumu[mu] * gmumu[nu];
          asymm_deriv += 0.25 * pow(hbarC * (surf[iel].dbeta[mu][nu] - surf[iel].dbeta[nu][mu]), 2) * gmumu[mu] * gmumu[nu];
          mod_deriv += pow(hbarC * (surf[iel].dbeta[mu][nu]), 2) * gmumu[mu] * gmumu[nu];
          // for(int rh=0; rh<4; rh++)
          // for(int sg=0; sg<4; sg++) {
