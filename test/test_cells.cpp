...
namespace
{
    ...
    TEST_F(CellTest, Write70Cells_Text)
    {
        const std::string i_file = "./input/beta.dat";
        std::ifstream file(i_file);
        const std::string o_file = "./input/beta-70.dat";
        std::ofstream outfile(o_file);
    ...
    TEST_F(CellTest, Read70Cells_Text)
    {
        const std::string i_file = "./input/beta-70.dat";
        std::ifstream file(i_file);

        std::string line;

        std::vector<hydro::fcell> cells;
        ...
        EXPECT_EQ(cells.size(), 70);
    }

    TEST_F(CellTest, Write60Cells_Bin)
    {
        const std::string i_file = "./input/beta.dat";
        std::ifstream file(i_file);
        const std::string o_file = "./input/beta-60.bin";
        std::ofstream outfile(o_file, std::ios::binary | std::ios::trunc);
        ...
        do
        {
            ...
            {
                iss >> cell;
                cell.write(outfile, 0, hydro::file_format::Binary);
                counter++;
            }
    ...
    TEST_F(CellTest, Write70Cells_Bin)
    {
        const std::string i_file = "./input/beta.dat";
        std::ifstream file(i_file);
        const std::string o_file = "./input/beta-70.bin";
        std::ofstream outfile(o_file, std::ios::binary | std::ios::trunc);
    ...
    TEST_F(CellTest, Read70Cells_Bin)
    {
...
#pragma omp critical
    ...
    TEST_F(CellTest, Test_Read_Surface_Text_60)
    {
        hydro::hypersurface<hydro::fcell> surface;
        surface.read("./input/beta-60.dat", utils::accept_modes::AcceptAll, true);
        EXPECT_EQ(surface.total(), 60);
        print(surface);
    }

    TEST_F(CellTest, Test_Read_Surface_Bin)
    {
        hydro::hypersurface<hydro::fcell> surface;
        surface.read("./input/beta-70.bin", utils::accept_modes::AcceptAll, true, hydro::file_format::Binary);
        print(surface);
        EXPECT_EQ(surface.total(), 70);
    }
}
